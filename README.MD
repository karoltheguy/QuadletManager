...
Download the source archive for the version you want from the
[Releases page](https://github.com/karoltheguy/QuadletManager/releases), or
clone and check out a release tag:
...
Images are published to `ghcr.io/karoltheguy/quadletmanager` for `linux/amd64`:

```bash
podman pull ghcr.io/karoltheguy/quadletmanager:0.1
```

| Tag | Points at |
|-----|-----------|
| `latest` | The newest release, whatever its version number |
| `0.1` | The newest patch in the `0.1.x` series |
| `0.1.0` | That exact release |
| `main` | Changes merged but not yet released. Most likely to break |
...
|----------|-------------|
| `QUADLET_MASTER_KEY` | AES-256 master key (64 hex characters). Falls back to an auto-generated dev key persisted in `master.key` if unset |
...
The rules themselves are listed and explained in
[docs/SETUP.MD](docs/SETUP.MD#3-global-quadlets-sudoers-setup).
...
> features fail on a server set up this way. See
> [issue #289](https://github.com/karoltheguy/QuadletManager/issues/289) and
> [docs/SUDO_PERMISSIONS.md](docs/SUDO_PERMISSIONS.md), which records every
> command the app actually runs under sudo.
...
how to run the four test suites the way CI does, and the pull request process.
