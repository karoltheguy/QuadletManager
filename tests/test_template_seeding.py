...
async def test_volume_template_has_volume_section(fresh_db):
    ...
    assert "[Volume]" in row[0], f"Volume template missing [Volume] section: {row[0]!r}"
...
async def test_network_template_has_network_section(fresh_db):
    ...
    assert "[Network]" in row[0], f"Network template missing [Network] section: {row[0]!r}"
...
async def test_pod_template_has_pod_section(fresh_db):
    ...
    assert "[Pod]" in row[0], f"Pod template missing [Pod] section: {row[0]!r}"
...
async def test_seeding_is_idempotent(fresh_db):
    ...
    assert row[0] == 4, f"Expected exactly 4 templates after two init_db() calls, got {row[0]}"
