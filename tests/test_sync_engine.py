...
class TestParseMtime(unittest.IsolatedAsyncioTestCase):
    """Tests for parse_mtime() — converts stat output to an integer timestamp."""

    async def test_valid_timestamp(self):
        """Standard numeric timestamp string parses correctly."""
        result = await parse_mtime("1709827200\n")
        self.assertEqual(result, 1709827200)

    async def test_timestamp_with_whitespace(self):
    ...
    async def test_non_numeric_returns_zero(self):
...
class TestCheckQuadlets(unittest.IsolatedAsyncioTestCase):
    """Tests for check_quadlets() — the core sync poller function.

    Uses the same mocking pattern as test_stats_engine.py.
    """

    @patch("services.sync_engine.publisher")
    @patch("services.sync_engine.pool")
    @patch("services.sync_engine.get_db_connection")
    async def test_newer_mtime_triggers_publish(self, mock_get_db_func, mock_pool, mock_publisher):
        ...
        self.assertEqual(call_args[0], "file_changed")
        self.assertEqual(call_args[1]["server_id"], 10)
        self.assertEqual(call_args[1]["file_path"], "/etc/containers/systemd/web.container")
        self.assertIn("message", call_args[1])

    @patch("services.sync_engine.publisher")
    @patch("services.sync_engine.pool")
    @patch("services.sync_engine.get_db_connection")
    async def test_same_mtime_no_publish(self, mock_get_db_func, mock_pool, mock_publisher):
    ...
    @patch("services.sync_engine.pool")
    @patch("services.sync_engine.get_db_connection")
    async def test_older_mtime_no_publish(self, mock_get_db_func, mock_pool, mock_publisher):
    ...
    @patch("services.sync_engine.pool")
    @patch("services.sync_engine.get_db_connection")
    async def test_none_mtime_no_publish(self, mock_get_db_func, mock_pool, mock_publisher):
    ...
    @patch("services.sync_engine.pool")
    @patch("services.sync_engine.get_db_connection")
    async def test_ssh_error_does_not_crash(self, mock_get_db_func, mock_pool, mock_publisher):
    ...
    @patch("services.sync_engine.pool")
    @patch("services.sync_engine.get_db_connection")
    async def test_user_scope_does_not_use_sudo(self, mock_get_db_func, mock_pool, mock_publisher):
    ...
    @patch("services.sync_engine.pool")
    @patch("services.sync_engine.get_db_connection")
    async def test_global_scope_uses_sudo(self, mock_get_db_func, mock_pool, mock_publisher):
    ...
    @patch("services.sync_engine.pool")
    @patch("services.sync_engine.get_db_connection")
    async def test_no_quadlets_in_db_is_noop(self, mock_get_db_func, mock_pool, mock_publisher):
    ...
    @patch("services.sync_engine.pool")
    @patch("services.sync_engine.get_db_connection")
    async def test_db_updated_with_new_mtime(self, mock_get_db_func, mock_pool, mock_publisher):
        ...
        self.assertIn("UPDATE quadlets SET last_known_mtime", sql)
        self.assertEqual(params, (3000, 8))
        update_db.commit.assert_called_once()


class TestCollisionAvoidance(unittest.IsolatedAsyncioTestCase):
    """Tests for the collision avoidance logic in sync_engine.

    After a UI save updates the mtime in the DB, the sync poller
    should NOT flag the file as externally modified.
    """

    @patch("services.sync_engine.publisher")
    @patch("services.sync_engine.pool")
    @patch("services.sync_engine.get_db_connection")
    async def test_after_save_updates_mtime_no_false_positive(self, mock_get_db_func, mock_pool, mock_publisher):
