...
**Monitor accessibility invariants** (issue #262): the Monitor pane signals health state without relying on colour, and two details are load-bearing. First, the `#monitor-health-status` live region (`role="status"`) sits **outside** `#monitor-stat-bar` in `dashboard.html`: the stat bar renders with `display:none` until a server is selected, and assistive tech never announces a live region inside a hidden subtree, so moving that span into the bar silently disables announcements. Second, `updateSummaryStrip` writes the region only when the unhealthy count differs from the module-level `lastAnnouncedUnhealthy` tracker. That guard reads like removable ceremony but is the entire design: the function runs on every SSE stats tick, so an unguarded write would make a screen reader recite the same figures continuously, which is worse than silence. Threshold styling follows the same principle -- `applyPercentSeverity` pairs every `cell-warn`/`cell-danger` colour with an `aria-hidden` glyph and a `visually-hidden` severity word, so no state is conveyed by colour alone.
